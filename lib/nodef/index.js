...
function applySnippet(tag) {
  var callable = tag.snippet.callable,
      el = stripSnippetTag(tag.element),
  ...
  return when(SnippetRegistry.get(callable))(
    match(some(objectSnippet))(
      function (o) { o.apply(el, args); return document }
    ),
    match(some(functionSnippet))(
      function (f) { f(el, args); return document }
    ),
    match(true)(
      function () { notRegistered(el, {snippet: callable }); return document }
    )
  );
}
